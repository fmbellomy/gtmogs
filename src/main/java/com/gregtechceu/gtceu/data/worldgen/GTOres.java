...
public class GTOres {
    ...
    private static Supplier<? extends Block> ore(TagPrefix oreTag, Material material) {
        ...
        if (block == null) {
            ...
            if (oreTag == ore) {
                oreKey = ResourceLocation.parse("%s_ore".formatted(material.getName()));
            } else if (oreTag == oreNetherrack) {
                oreKey = ResourceLocation.parse("nether_%s_ore".formatted(material.getName()));
            } else {
                oreKey = ResourceLocation.parse("%s_%s_ore".formatted(oreTag.name, material.getName()));
            }
