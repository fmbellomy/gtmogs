...
public class RubberTrunkPlacer extends TrunkPlacer {
    ...
    public List<FoliagePlacer.FoliageAttachment> placeTrunk(LevelSimulatedReader level,
        ...
        for (int i = 0; i < freeTreeHeight; ++i) {
            ...
            if (i == 1 || i == freeTreeHeight - 7) {
                placeLogIfFreeWithOffsetAndChance(0.33f, level, blockSetter, random, mutableBlockPos, config, pos, 1, i,
                        1);
                placeLogIfFreeWithOffsetAndChance(0.33f, level, blockSetter, random, mutableBlockPos, config, pos, 1, i,
                        -1);
                placeLogIfFreeWithOffsetAndChance(0.33f, level, blockSetter, random, mutableBlockPos, config, pos, -1,
                        i, 1);
                placeLogIfFreeWithOffsetAndChance(0.33f, level, blockSetter, random, mutableBlockPos, config, pos, -1,
                        i, -1);
    ...
    private void placeLogIfFreeWithOffset(LevelSimulatedReader level, BiConsumer<BlockPos, BlockState> blockSetter,
                                          RandomSource random, BlockPos.MutableBlockPos pos, TreeConfiguration config,
    ...
    private void placeLogIfFreeWithOffsetAndChance(float pChance, LevelSimulatedReader level,
                                                   BiConsumer<BlockPos, BlockState> blockSetter, RandomSource random,
                                                   BlockPos.MutableBlockPos pos, TreeConfiguration config,
                                                   BlockPos pOffsetPos, int pOffsetX, int pOffsetY, int pOffsetZ) {
