...
public class BlastingRecipeBuilder {
    ...
    public BlastingRecipeBuilder input(ItemStack itemStack) {
        if (!itemStack.isComponentsPatchEmpty() || itemStack.getDamageValue() > 0) {
    ...
    public void save(RecipeOutput output) {
        ...
        ID = ResourceLocation.fromNamespaceAndPath(ID.getNamespace(), "blasting" + "/" + ID.getPath());;
        output.accept(ID, );
        output.accept(new FinishedRecipe() {
            ...
            public ResourceLocation getId() {
                var ID = id == null ? defaultId() : id;
                return ResourceLocation.fromNamespaceAndPath(ID.getNamespace(), "blasting" + "/" + ID.getPath());
            }
